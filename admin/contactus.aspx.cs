    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Page.IsPostBack == false)
        {
            if (Request["id"] != null)
            {
                ...
                SqlDataSource1.DeleteCommand = "Delete from tbl_contact where Id=@Id";
                ...
                try
                {
                    ...
                    if (n > 0)
                    {
                        Session["msg"] = "Contact Message is Deleted";
                    }
                    else
                    {
                        Session["msg"] = "Contact Message is not Deleted";
                    }
                }
                catch (Exception ex)
                {
            ...
            SqlDataSource1.ConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings["ShopperDbConnect"].ToString();
            SqlDataSource1.SelectCommand = "Select * from tbl_contact order by Id";
            try
            {
